...
class Oberflaeche extends React.Component {
  ...
  valsErfassen(k) {
    ...
    return {
      ...
      onScanKameraBereit: (video) => { this._scanVideoEl = video; this.scanAutoBeobachten(video); },
      scanKameraSchiessen: () => this.scanKameraAufnehmen(this._scanVideoEl),
      scanKameraZuDatei: () => this.scanZuDatei(),
      scanAutoOn: !!(s.scan && s.scan.autoCapture),
      scanAutoText: (s.scan && s.scan.autoStatus) || '',
      scanAutoToggle: () => this.scanAutoUmschalten(),
