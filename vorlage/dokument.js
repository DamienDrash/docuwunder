...
(function (global) {
  ...
  <div style=${stil('position:absolute;inset:0;overflow-y:auto;padding:110px 0 120px')}>
    <div style=${stil('display:flex;background:var(--fill);border-radius:10px;margin:0 16px;padding:2px')}>
      <button type="button" aria-label="Vorschau anzeigen" aria-pressed=${!!v.dSegV} onClick=${v.setSegV} style=${stil(S.buttonReset + v.sgV)}>Vorschau</button>
      <button type="button" aria-label="Erkannten Text anzeigen" aria-pressed=${!!v.dSegT} onClick=${v.setSegT} style=${stil(S.buttonReset + v.sgT)}>Text</button>
      <button type="button" aria-label="Informationen anzeigen" aria-pressed=${!!v.dSegI} onClick=${v.setSegI} style=${stil(S.buttonReset + v.sgI)}>Info</button>
    </div>
    ${v.dSegV ? html`${v.dFundOn ? html`<div style=${stil('margin:12px 16px 0;background:var(--accT);border-radius:10px;padding:9px 12px;display:flex;align-items:center;gap:8px')}>
      ...
      ${v.dPrevOn ? html`<button type="button" aria-label=${`Vorschau von ${v.dTitel} öffnen`} onClick=${v.dOpenFile} style=${stil(S.buttonReset + 'display:block;width:100%;margin:14px 26px 0;background:var(--pg);border:0.5px solid var(--sep);border-radius:6px;box-shadow:0 10px 26px rgba(0,0,0,0.10);overflow:hidden;cursor:pointer')}>
    ...
    ${v.dSegI ? html`<div style=${stil('background:var(--card);border-radius:16px;margin:14px 16px 0;overflow:hidden')}>
        <button type="button" aria-label="Absender ändern" onClick=${v.openEdit} style=${stil(S.buttonReset + 'width:100%;display:flex;justify-content:space-between;align-items:center;padding:11px 16px;cursor:pointer;position:relative')}><span style=${stil(S.nebenGross)}>Absender</span><span style=${stil(S.zeileText)}>${v.dAbs}</span><div style=${stil(S.trenner)}></div></button>
        <button type="button" aria-label="Dokumentart ändern" onClick=${v.openEdit} style=${stil(S.buttonReset + 'width:100%;display:flex;justify-content:space-between;align-items:center;padding:11px 16px;cursor:pointer;position:relative')}><span style=${stil(S.nebenGross)}>Dokumentart</span><span style=${stil(S.zeileText)}>${v.dArt}</span><div style=${stil(S.trenner)}></div></button>
      ...
      <div style=${stil('background:var(--card);border-radius:16px;margin:12px 16px 0;padding:12px 16px')}>
        ...
        <div style=${stil('display:flex;gap:6px;flex-wrap:wrap;margin-top:8px')}>
          ...
          <button type="button" aria-label="Schlagwörter ändern" onClick=${v.openEdit} style=${stil(S.buttonReset + 'font-size:12.5px;font-weight:600;color:var(--acc);background:var(--accT);padding:5px 11px;border-radius:8px;cursor:pointer')}>Ändern</button>
        </div>
      </div>
      <div style=${stil('background:var(--card);border-radius:16px;margin:12px 16px 0;overflow:hidden')}>
        <button type="button" aria-label="Original herunterladen" onClick=${v.dmDownload} style=${stil(S.buttonReset + 'width:100%;display:flex;align-items:center;gap:10px;padding:12px 16px;cursor:pointer;position:relative')}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3.5v11"></path><path d="M8.2 10.7l3.8 3.8 3.8-3.8"></path><path d="M5 20.5h14"></path></svg><span style=${stil('font-size:15.5px;color:var(--acc)')}>Original herunterladen</span><div style=${stil('position:absolute;left:43px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
        <button type="button" aria-label="Drucken" onClick=${v.dmPrint} style=${stil(S.buttonReset + 'width:100%;display:flex;align-items:center;gap:10px;padding:12px 16px;cursor:pointer')}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M7 8V3.5h10V8"></path><rect x="4" y="8" width="16" height="8" rx="1.5"></rect><path d="M7 13.5h10v7H7z"></path></svg><span style=${stil('font-size:15.5px;color:var(--acc)')}>Drucken</span></button>
      </div>` : null}
  </div>
  <div style=${stil(S.kopf)}>
    ...
    <button type="button" aria-label="Teilen" onClick=${v.openShare} style=${stil(S.buttonReset + S.kopfKnopf)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 14.5V3.5"></path><path d="M8.2 7L12 3.3 15.8 7"></path><path d="M6.5 11H5v9.5h14V11h-1.5"></path></svg></button>
    <button type="button" aria-label="Weitere Aktionen" onClick=${v.openDocMenu} style=${stil(S.buttonReset + S.kopfKnopf)}><svg width="16" height="16" viewBox="0 0 24 24" fill="var(--acc)" stroke="none"><circle cx="5" cy="12" r="2"></circle><circle cx="12" cy="12" r="2"></circle><circle cx="19" cy="12" r="2"></circle></svg></button>
  </div>
  <div style=${stil(`position:absolute;${v.barPosDoc}bottom:24px;height:60px;border-radius:999px;background:var(--glass);backdrop-filter:blur(18px) saturate(180%);-webkit-backdrop-filter:blur(18px) saturate(180%);border:0.5px solid var(--gbor);box-shadow:0 8px 28px rgba(0,0,0,0.14);display:flex;z-index:30`)}>
    <button type="button" aria-label=${v.dFav ? 'Als Favorit entfernen' : 'Als Favorit markieren'} aria-pressed=${!!v.dFav} onClick=${v.dToggleFav} style=${stil(S.buttonReset + 'flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;color:var(--acc)')}>
    ...
    <button type="button" aria-label="Bearbeiten" onClick=${v.openEdit} style=${stil(S.buttonReset + 'flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;color:var(--acc)')}>
    ...
    <button type="button" aria-label="Teilen" onClick=${v.openShare} style=${stil(S.buttonReset + 'flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;color:var(--acc)')}>
    ...
    <button type="button" aria-label="Löschen" onClick=${v.dmTrash} style=${stil(S.buttonReset + 'flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;color:var(--red)')}>
  ...
  <div style=${stil(`position:absolute;inset:0;overflow-y:auto;padding:110px 0 130px;transform:translateX(${Math.round(v.revDx)}px);transition:${v.revZiehAn ? 'none' : 'transform .22s cubic-bezier(.3,.7,.4,1)'};${v.revZiehAn ? 'user-select:none;-webkit-user-select:none;' : ''}`)}>
    ...
    <div style=${stil('display:flex;align-items:center;justify-content:space-between;padding:20px 20px 8px')}>
      ...
      <button type="button" aria-label="Alle erkannten Angaben übernehmen" onClick=${v.acceptAll} style=${stil(S.buttonReset + 'font-size:13.5px;font-weight:600;color:var(--acc);cursor:pointer')}>Alle übernehmen</button>
    </div>
    <div style=${stil(S.karte)}>
      ${(v.revFields || []).map((f, fIdx) => html`<div key=${f && f.id != null ? f.id : fIdx} style=${stil('display:flex;align-items:center;gap:12px;padding:10px 16px;position:relative')}>
          <button type="button" aria-label=${`${f.k} ${f.ok ? 'übernommen, tippen zum Verwerfen' : 'nicht übernommen, tippen zum Übernehmen'}`} aria-pressed=${!!f.ok} onClick=${f.toggle} style=${stil(S.buttonReset + 'cursor:pointer;flex-shrink:0')}>
          ...
          <button type="button" aria-label=${`${f.k} bearbeiten`} onClick=${f.edit} style=${stil(S.buttonReset + 'flex:1;min-width:0;cursor:pointer;text-align:left')}>
  ...
  <div style=${stil(S.kopf)}>
    ...
    <div style=${stil('flex:1;display:flex;align-items:center;justify-content:center;gap:10px')}>
      ${v.revMehrere ? html`<button type="button" aria-label="Vorheriges Dokument" disabled=${!v.revVor} onClick=${v.revZurueckTap} style=${stil(S.buttonReset + 'width:26px;height:26px;border-radius:999px;display:flex;align-items:center;justify-content:center;'
        + (v.revVor ? 'cursor:pointer;background:var(--fill)' : 'opacity:0.28'))}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--lab2)" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M15 5l-7 7 7 7"></path></svg></button>` : null}
      <span style=${stil(S.kopfTitel)}>${v.revPos}</span>
      ${v.revMehrere ? html`<button type="button" aria-label="Nächstes Dokument" disabled=${!v.revZurueck} onClick=${v.revVorTap} style=${stil(S.buttonReset + 'width:26px;height:26px;border-radius:999px;display:flex;align-items:center;justify-content:center;'
        + (v.revZurueck ? 'cursor:pointer;background:var(--fill)' : 'opacity:0.28'))}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--lab2)" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M9 5l7 7-7 7"></path></svg></button>` : null}
    </div>
    <button type="button" aria-label="Dokument verwerfen" onClick=${v.revDelete} style=${stil(S.buttonReset + S.kopfKnopf)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 6.5h15"></path><path d="M8.5 6.2V4.5h7v1.7"></path><path d="M6.5 6.5l1 14h9l1-14"></path></svg></button>
  </div>
  ...
  <div style=${stil('position:absolute;top:0;left:0;right:0;z-index:6;display:flex;align-items:center;gap:10px;padding:60px 16px 10px;background:var(--bg)')}>
    <div style=${stil('flex:1;display:flex;gap:8px;align-items:center;background:var(--fill);border-radius:12px;padding:0 12px;height:40px')}>
      ...
      <label for="dw-suchfeld" style=${stil('position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0)')}>Titel, Absender oder Inhalt durchsuchen</label>
      <input id="dw-suchfeld" value=${v.qVal} onInput=${v.setQ} placeholder="Titel, Absender oder Inhalt" style=${stil('flex:1;min-width:0;border:none;background:transparent;outline:none;font-size:16px;color:var(--lab)')} />
      ${v.hasQ ? html`<button type="button" aria-label="Suchtext löschen" onClick=${v.qClear} style=${stil(S.buttonReset + 'width:18px;height:18px;border-radius:50%;background:var(--fill2);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0')}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="var(--lab2)" stroke-width="3" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"></path></svg></button>` : null}
    </div>
    <button type="button" aria-label="Suche abbrechen" onClick=${v.cancelSearch} style=${stil(S.buttonReset + 'font-size:16px;color:var(--acc);cursor:pointer;flex-shrink:0')}>Abbrechen</button>
  </div>
  <div style=${stil('position:absolute;inset:0;overflow-y:auto;padding:118px 0 40px')}>
    ${v.noQ ? html`<div style=${stil('display:flex;gap:8px;flex-wrap:wrap;padding:4px 16px 0')}>
        ${(v.qChips || []).map((c, cIdx) => html`<button type="button" key=${c && c.id != null ? c.id : cIdx} aria-label=${`Nach ${c.label} filtern`} onClick=${c.tap} style=${stil(S.buttonReset + 'height:32px;padding:0 13px;border-radius:999px;background:var(--fill);display:flex;align-items:center;font-size:13.5px;font-weight:600;cursor:pointer')}>${c.label}</button>`)}
      </div>
      <div style=${stil('display:flex;align-items:center;justify-content:space-between;padding:20px 20px 8px')}>
        ...
        <button type="button" aria-label="Zuletzt gesuchte Begriffe löschen" onClick=${v.clearRecents} style=${stil(S.buttonReset + 'font-size:13.5px;font-weight:600;color:var(--acc);cursor:pointer')}>Löschen</button>
      </div>
      <div style=${stil(S.karte)}>
        ${(v.recentsRows || []).map((r, rIdx) => html`<button type="button" key=${r && r.id != null ? r.id : rIdx} aria-label=${`Erneut nach „${r.q}“ suchen`} onClick=${r.tap} style=${stil(S.buttonReset + 'width:100%;' + S.zeile)}>
      ...
      <div style=${stil(S.karte)}>
        ${(v.savedRows || []).map((r, rIdx) => html`<button type="button" key=${r && r.id != null ? r.id : rIdx} aria-label=${`Gespeicherte Suche „${r.name}“ öffnen`} onClick=${r.tap} style=${stil(S.buttonReset + 'width:100%;' + S.zeile)}>
    ...
    ${v.hasQ ? html`<div style=${stil('font-size:12.5px;color:var(--lab3);padding:2px 20px 8px')}>${v.resCountLabel}</div>
      <div style=${stil(S.karte)}>
        ${(v.resRows || []).map((r, rIdx) => html`<button type="button" key=${r && r.id != null ? r.id : rIdx} aria-label=${`Dokument „${r.titel}“ öffnen`} onClick=${r.open} style=${stil(S.buttonReset + 'width:100%;text-align:left;padding:11px 16px;cursor:pointer;position:relative')}>
      ...
      ${v.canSave ? html`<button type="button" aria-label="Suche speichern" onClick=${v.saveSearch} style=${stil(S.buttonReset + 'width:calc(100% - 32px);margin:12px 16px 0;height:44px;border-radius:14px;background:var(--fill);display:flex;align-items:center;justify-content:center;gap:7px;font-size:15px;font-weight:600;color:var(--acc);cursor:pointer')}>Suche speichern</button>` : null}` : null}
  </div>
</div>` : null;
  };
