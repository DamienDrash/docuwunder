...
(function (global) {
  ...
  ${v.ob0 ? html`<div style=${stil('position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;text-align:center;padding:120px 32px 40px')}>
      ...
      <button type="button" aria-label="Einrichtung starten" onClick=${v.obStart} style=${stil(S.buttonReset + ';width:100%;height:50px;border-radius:14px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer;box-shadow:0 6px 18px rgba(13,27,42,0.18)')}>Los geht’s</button>
  ...
  ${v.ob1 ? html`<div style=${stil('position:absolute;inset:0;overflow-y:auto;padding:70px 24px 40px')}>
      ...
      <button type="button" aria-expanded=${v.onbShowAdv ? 'true' : 'false'} onClick=${v.obToggleAdv} style=${stil(S.buttonReset + ';display:flex;align-items:center;gap:6px;margin-top:20px;cursor:pointer')}>
      ...
      <button type="button" onClick=${v.obConnect} style=${stil(S.buttonReset + ';width:100%;margin-top:26px;height:50px;border-radius:14px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;gap:8px;font-size:16.5px;font-weight:600;cursor:pointer')}>
  ...
  ${v.ob2 ? html`<div style=${stil('position:absolute;inset:0;overflow-y:auto;padding:70px 24px 40px')}>
      ...
      ${v.onbUserMode ? html`<div style=${stil('font-size:12.5px;font-weight:600;color:var(--lab2);text-transform:uppercase;letter-spacing:0.3px;margin-top:24px')}>Benutzername</div>
        ...
        <input aria-label="Passwort" type="password" value=${v.onbPassVal} onInput=${v.setOnbPass} placeholder="••••••••" style=${stil('width:100%;height:50px;border-radius:12px;border:1px solid var(--sep);background:var(--card);padding:0 14px;font-size:16px;color:var(--lab);outline:none;margin-top:8px')} style-focus="border-color:var(--acc)" />` : null}
      ...
      <button type="button" onClick=${v.obLoginTap} style=${stil(S.buttonReset + ';width:100%;margin-top:24px;height:50px;border-radius:14px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;gap:8px;font-size:16.5px;font-weight:600;cursor:pointer')}>
        ${v.onbChecking ? html`<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" style=${stil('animation:spin 0.9s linear infinite')}><path d="M12 3a9 9 0 109 9"></path></svg>` : null}
        <span>Anmelden</span>
      </button>
      <button type="button" onClick=${v.obSSO} style=${stil(S.buttonReset + ';width:100%;margin-top:10px;height:46px;border-radius:14px;background:var(--fill);display:flex;align-items:center;justify-content:center;font-size:15.5px;font-weight:600;color:var(--acc);cursor:pointer')}>Mit Single Sign-On anmelden</button>
      <button type="button" onClick=${v.obToggleTok} style=${stil(S.buttonReset + ';width:100%;text-align:center;margin-top:16px;font-size:14.5px;font-weight:600;color:var(--acc);cursor:pointer')}>${v.obTokLinkLabel}</button>
    </div>` : null}
  ${v.ob3 ? html`<div style=${stil('position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;text-align:center;padding:150px 32px 40px')}>
      ...
      <div style=${stil('flex:1')}></div>
      <button type="button" onClick=${v.obFertig} style=${stil(S.buttonReset + ';width:100%;height:50px;border-radius:14px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer')}>Los geht’s</button>
    </div>` : null}
</div>` : null;
  };
