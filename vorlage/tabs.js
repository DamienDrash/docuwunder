...
(function (global) {
  ...
  <button type="button" aria-label="Auswahl verschlagworten" onClick=${v.bulkTagOpen} style=${stil(S.buttonReset + ';' + v.selActStyle)}><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 12V4.5H11l9 9L12.5 21z"></path><circle cx="7.6" cy="8.6" r="1.3"></circle></svg></button>
  <button type="button" aria-label="Auswahl exportieren" onClick=${v.bulkExport} style=${stil(S.buttonReset + ';' + v.selActStyle)}><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 14.5V3.5"></path><path d="M8.2 7L12 3.3 15.8 7"></path><path d="M6.5 11H5v9.5h14V11h-1.5"></path></svg></button>
  <button type="button" aria-label="Auswahl löschen" onClick=${v.bulkTrash} style=${stil(S.buttonReset + ';' + v.selActStyleRed)}><svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 6.5h15"></path><path d="M8.5 6.2V4.5h7v1.7"></path><path d="M6.5 6.5l1 14h9l1-14"></path><path d="M10.2 10.5v6M13.8 10.5v6"></path></svg></button>
  <button type="button" onClick=${v.toggleSelMode} style=${stil(S.buttonReset + ';height:38px;padding:0 14px;border-radius:999px;display:flex;align-items:center;font-size:14.5px;font-weight:600;color:var(--acc);cursor:pointer')}>Fertig</button>
</div>` : null;
  };
  ...
  <div style=${stil('display:flex;align-items:center;justify-content:space-between;padding:8px 16px 0 20px')}>
    ...
    <div style=${stil('display:flex;gap:8px;align-items:center')}>
      <button type="button" onClick=${v.toggleSelMode} style=${stil(S.buttonReset + ';height:34px;padding:0 13px;border-radius:999px;background:var(--fill);display:flex;align-items:center;font-size:14px;font-weight:600;color:var(--acc);cursor:pointer')}>${v.selBtnLabel}</button>
      <button type="button" aria-label="Dokument hinzufügen" onClick=${v.openAdd} style=${stil(S.buttonReset + ';width:34px;height:34px;border-radius:50%;background:var(--acc);display:flex;align-items:center;justify-content:center;cursor:pointer')}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--onAcc)" stroke-width="2.2" stroke-linecap="round"><path d="M12 5v14M5 12h14"></path></svg></button>
    </div>
  </div>
  <button type="button" aria-label="Dokumente durchsuchen" onClick=${v.openSearch} style=${stil(S.buttonReset + ';margin:14px 16px 0;display:flex;gap:8px;align-items:center;background:var(--fill);border-radius:12px;padding:10px 12px;color:var(--lab2);cursor:pointer;width:calc(100% - 32px);text-align:left')}>
  ...
  <div style=${stil('display:flex;gap:8px;align-items:center;padding:12px 16px 4px')}>
    ...
    ${v.viewIsOrdner ? null : html`<button type="button" aria-label="Filter öffnen" onClick=${v.openFilter} style=${stil(S.buttonReset + ';height:32px;padding:0 11px;border-radius:999px;background:var(--fill);display:flex;align-items:center;gap:6px;flex-shrink:0;cursor:pointer')}>
    ...
    ${(v.filterChips || []).map((c, cIdx) => html`<button type="button" key=${c && c.id != null ? c.id : cIdx} aria-pressed=${c.on ? "true" : "false"} onClick=${c.tap} style=${stil(S.buttonReset + ';' + c.style)}><span>${c.label}</span></button>`)}
    <button type="button" aria-label="Sortierung ändern" onClick=${v.openSort} style=${stil(S.buttonReset + ';height:32px;padding:0 11px;border-radius:999px;background:var(--fill);display:flex;align-items:center;gap:5px;flex-shrink:0;cursor:pointer')}>
    ...
    <button type="button" aria-label="Ansicht wechseln" onClick=${v.toggleView} style=${stil(S.buttonReset + ';width:32px;height:32px;border-radius:999px;background:var(--fill);display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer')}>
  ...
  ${v.viewIsList ? html`<div style=${stil(S.karte)}>
      ${(v.visDocs || []).map((d, dIdx) => html`<div key=${d && d.id != null ? d.id : dIdx} style=${stil('position:relative;overflow:hidden')}>
        <div style=${stil('position:absolute;top:0;bottom:0;right:0;display:flex')}>
          <button type="button" aria-label="Favorit umschalten" onClick=${d.swFav} style=${stil(S.buttonReset + ';width:68px;background:#F7B500;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;cursor:pointer;color:#fff')}>
          ...
          <button type="button" aria-label="Dokument löschen" onClick=${d.swDel} style=${stil(S.buttonReset + ';width:68px;background:var(--red);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;cursor:pointer;color:#fff')}>
        ...
        <button type="button" aria-label=${`Dokument „${d.titel}“ öffnen`} onClick=${d.tap} onPointerDown=${d.pd} onPointerMove=${d.pm} onPointerUp=${d.pu} onPointerLeave=${d.pu} style=${stil(S.buttonReset + ';display:block;width:100%;text-align:left;' + d.rowStyle)}>
  ...
  ${v.viewIsOrdner ? html`<div>
    ...
    <div style=${stil('display:flex;align-items:center;gap:4px;padding:0 16px 10px;overflow-x:auto;white-space:nowrap')}>
      ...
      ${(v.otWeg || []).map((w, wIdx) => html`<span key=${wIdx} style=${stil('display:flex;align-items:center;gap:4px;flex-shrink:0')}>
        ...
        ${w.letzter ? html`<span style=${stil('font-size:13.5px;font-weight:600;color:var(--lab)')}>${w.name}</span>` : html`<button type="button" onClick=${w.tap} style=${stil(S.buttonReset + ';font-size:13.5px;font-weight:500;color:var(--acc);cursor:pointer')}>${w.name}</button>`}
      </span>`)}
    </div>
    <div style=${stil(S.karte)}>
      ${(v.otOrdner || []).map((f, fIdx) => html`<button type="button" key=${'o' + fIdx} aria-label=${`Ordner „${f.name}“ öffnen`} onClick=${f.tap} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:12px 16px;cursor:pointer;position:relative')}>
      ...
      ${(v.otDateien || []).map((d, dIdx) => html`<button type="button" key=${d && d.id != null ? d.id : 'd' + dIdx} aria-label=${`Dokument „${d.titel}“ öffnen`} onClick=${d.open} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:11px;padding:10px 16px;cursor:pointer;position:relative')}>
  ...
  ${v.viewIsGrid ? html`<div style=${stil('display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:0 16px')}>
      ${(v.visDocs || []).map((d, dIdx) => html`<button type="button" key=${d && d.id != null ? d.id : dIdx} aria-label=${`Dokument „${d.titel}“ öffnen`} onClick=${d.tap} style=${stil(S.buttonReset + ';display:block;width:100%;text-align:left;background:var(--card);border-radius:14px;padding:10px;cursor:pointer;position:relative')}>
  ...
  ${v.docsMoreOn ? html`<button type="button" onClick=${v.loadMore} style=${stil(S.buttonReset + ';width:calc(100% - 32px);margin:14px 16px 0;height:44px;border-radius:14px;background:var(--fill);display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:600;color:var(--acc);cursor:pointer')}>${v.docsMoreLabel}</button>` : null}
  ...
  ${v.docsErrOn ? html`<div style=${stil('margin:14px 16px 0;background:var(--card);border-radius:14px;padding:14px 16px;text-align:center')}>
      ...
      <button type="button" onClick=${v.retryLoad} style=${stil(S.buttonReset + ';display:inline-flex;margin-top:12px;height:38px;padding:0 18px;border-radius:999px;background:var(--fill);align-items:center;font-size:14.5px;font-weight:600;color:var(--acc);cursor:pointer')}>Erneut versuchen</button>
    </div>` : null}
  ...
  <div style=${stil('display:flex;align-items:center;justify-content:space-between;padding:8px 20px 0')}>
    ...
    <button type="button" aria-label="Einstellungen öffnen" onClick=${v.openSettings} style=${stil(S.buttonReset + ';width:36px;height:36px;border-radius:50%;background:var(--fill2);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;color:var(--lab2);cursor:pointer')}>${v.kontoInitialen}</button>
  </div>
  <button type="button" aria-label="Dokumente durchsuchen" onClick=${v.openSearch} style=${stil(S.buttonReset + ';margin:14px 16px 0;display:flex;gap:8px;align-items:center;background:var(--fill);border-radius:12px;padding:10px 12px;color:var(--lab2);cursor:pointer;width:calc(100% - 32px);text-align:left')}>
    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="7"></circle><path d="M16.8 16.8L21 21"></path></svg>
  ...
  <div style=${stil('display:flex;gap:10px;margin:12px 16px 0')}>
    <button type="button" onClick=${v.startScan} style=${stil(S.buttonReset + ';flex:1;background:var(--accT);border-radius:14px;padding:12px 8px;display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer')}>
    ...
    <button type="button" onClick=${v.pickFile} style=${stil(S.buttonReset + ';flex:1;background:var(--card);border-radius:14px;padding:12px 8px;display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer')}>
  ...
  <div style=${stil(S.karte)}>
    ${v.inboxHasNew ? html`<button type="button" aria-label="Posteingang prüfen" onClick=${v.goInbox} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:12px 16px;cursor:pointer;position:relative')}>
    ...
    ${v.dupOn ? html`<button type="button" aria-label="Mögliches Duplikat prüfen" onClick=${v.openDup} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:12px 16px;cursor:pointer;position:relative')}>
    ...
    ${v.missOn ? html`<button type="button" aria-label="Angaben vervollständigen" onClick=${v.openMissing} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:12px 16px;cursor:pointer')}>
  ...
  <div style=${stil('display:flex;gap:12px;overflow-x:auto;padding:2px 16px 6px')}>
    ${(v.recentDocs || []).map((r, rIdx) => html`<button type="button" key=${r && r.id != null ? r.id : rIdx} aria-label=${`Dokument „${r.titel}“ öffnen`} onClick=${r.open} style=${stil(S.buttonReset + ';display:block;text-align:left;width:122px;flex-shrink:0;background:var(--card);border-radius:14px;padding:10px;cursor:pointer')}>
  ...
  <div style=${stil(S.karte)}>
    ${(v.lastOpened || []).map((r, rIdx) => html`<button type="button" key=${r && r.id != null ? r.id : rIdx} aria-label=${`Dokument „${r.titel}“ öffnen`} onClick=${r.open} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:10px 16px;cursor:pointer;position:relative')}>
  ...
  <div style=${stil(S.karte)}>
    ${(v.favDocs || []).map((r, rIdx) => html`<button type="button" key=${r && r.id != null ? r.id : rIdx} aria-label=${`Dokument „${r.titel}“ öffnen`} onClick=${r.open} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:10px 16px;cursor:pointer;position:relative')}>
  ...
  ${v.inboxEmpty ? html`<div style=${stil('padding:80px 40px;text-align:center')}>
      ...
      <button type="button" onClick=${v.startScan} style=${stil(S.buttonReset + ';display:inline-flex;margin-top:18px;height:42px;padding:0 20px;border-radius:999px;background:var(--acc);align-items:center;gap:7px;font-size:15px;font-weight:600;color:var(--onAcc);cursor:pointer')}>
  ...
  <div style=${stil('margin-top:14px')}>
    ${(v.inboxList || []).map((it, itIdx) => html`<button type="button" key=${it && it.id != null ? it.id : itIdx} aria-label=${`Posteingang: „${it.titel}“ prüfen`} onClick=${it.review} style=${stil(S.buttonReset + ';width:calc(100% - 32px);text-align:left;background:var(--card);border-radius:16px;margin:0 16px 10px;padding:12px 14px;display:flex;gap:12px;align-items:center;cursor:pointer')}>
  ...
  <div style=${stil(S.karte)}>
    ${(v.bibRows || []).map((m, mIdx) => html`<button type="button" key=${m && m.id != null ? m.id : mIdx} aria-label=${m.label} onClick=${m.tap} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:11px 16px;cursor:pointer;position:relative')}>
  ...
  <div style=${stil(S.karte)}>
    ${(v.orgRows || []).map((m, mIdx) => html`<button type="button" key=${m && m.id != null ? m.id : mIdx} aria-label=${m.label} onClick=${m.tap} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:11px 16px;cursor:pointer;position:relative')}>
        <div style=${stil(m.iconStyle)}><span style=${stil('display:flex')} dangerouslySetInnerHTML=${m.svg}></span></div>
        <div style=${stil('flex:1;font-size:16px')}>${m.label}</div>
        <span style=${stil('font-size:15px;color:var(--lab2);margin-right:2px')}>${m.detail}</span>
        <svg width="8" height="14" viewBox="0 0 8 14" fill="none" stroke="var(--lab3)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l6 6-6 6"></path></svg>
  ...
  <div style=${stil(S.karte)}>
    ${(v.adminRows || []).map((m, mIdx) => html`<button type="button" key=${m && m.id != null ? m.id : mIdx} aria-label=${m.label} onClick=${m.tap} style=${stil(S.buttonReset + ';width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:11px 16px;cursor:pointer;position:relative')}>
        <div style=${stil(m.iconStyle)}><span style=${stil('display:flex')} dangerouslySetInnerHTML=${m.svg}></span></div>
        <div style=${stil('flex:1;font-size:16px')}>${m.label}</div>
        <span style=${stil('font-size:15px;color:var(--lab2);margin-right:2px')}>${m.detail}</span>
        <svg width="8" height="14" viewBox="0 0 8 14" fill="none" stroke="var(--lab3)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l6 6-6 6"></path></svg>
