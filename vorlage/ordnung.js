...
(function (global) {
  ...
  <div style=${stil(S.blatt)}>
    ...
    <div style=${stil(S.karte)}>
      ${(v.listeRows || []).map((d, dIdx) => html`<button type="button" key=${d && d.id != null ? d.id : dIdx} aria-label=${d.titel} onClick=${d.open} style=${stil(S.buttonReset + 'width:100%;display:flex;align-items:center;gap:11px;padding:10px 16px;cursor:pointer;position:relative')}>
          ...
          <div style=${stil('flex:1;min-width:0;text-align:left')}><div style=${stil('font-size:15.5px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>${d.titel}</div><div style=${stil('font-size:13px;color:var(--lab2);margin-top:1px')}>${d.sub}</div></div>
  ...
  <div style=${stil(S.kopf)}>
    ...
    <button type="button" aria-label="Hinzufügen" onClick=${v.orgAdd} style=${stil(S.buttonReset + S.kopfKnopf)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="2.2" stroke-linecap="round"><path d="M12 5v14M5 12h14"></path></svg></button>
  </div>
  <div style=${stil(S.blatt)}>
    <div style=${stil('font-size:12.5px;color:var(--lab3);padding:0 20px 10px')}>${v.orgHint}</div>
    ${v.ordnerAn ? html`<div style=${stil(S.karte)}>
        ${(v.ordnerListe || []).map((f, fIdx) => html`<button type="button" key=${f && f.id != null ? f.id : fIdx} aria-label=${f.name} onClick=${f.tap} style=${stil(S.buttonReset + 'width:100%;' + S.zeile)}>
            ...
            <div style=${stil('flex:1;min-width:0;text-align:left')}><div style=${stil('font-size:15.5px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>${f.name}</div><div style=${stil('font-size:13px;color:var(--lab2);margin-top:1px')}>${f.countLabel}</div></div>
        ...
        ${(v.ordnerDateien || []).map((d, dIdx) => html`<button type="button" key=${d && d.id != null ? d.id : dIdx} aria-label=${d.titel} onClick=${d.open} style=${stil(S.buttonReset + 'width:100%;' + S.zeile)}>
            ...
            <div style=${stil('flex:1;min-width:0;text-align:left')}><div style=${stil('font-size:15.5px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>${d.titel}</div><div style=${stil('font-size:13px;color:var(--lab2);margin-top:1px')}>${d.sub}</div></div>
            <div style=${stil(S.trenner)}></div>
          </button>`)}
      </div>
    ...
    ${v.ordnerAus ? html`<div style=${stil(S.karte)}>
      ${(v.orgList || []).map((o, oIdx) => html`<button type="button" key=${o && o.id != null ? o.id : oIdx} aria-label=${o.name} onClick=${o.tap} style=${stil(S.buttonReset + 'width:100%;' + S.zeile)}>
          <div style=${stil('flex:1;min-width:0;text-align:left')}><div style=${stil('font-size:15.5px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>${o.name}</div><div style=${stil('font-size:12.5px;color:var(--lab2);margin-top:1px')}>${o.countLabel}</div></div>
          <svg width="8" height="14" viewBox="0 0 8 14" fill="none" stroke="var(--lab3)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l6 6-6 6"></path></svg>
          <div style=${stil(S.trenner)}></div>
        </button>`)}
    </div>` : null}
    ...
    <div style=${stil(S.karte)}>
      ${(v.trashRows || []).map((d, dIdx) => html`<div key=${d && d.id != null ? d.id : dIdx} style=${stil('padding:12px 16px;position:relative')}>
          ...
          <div style=${stil('display:flex;gap:8px;margin-top:10px')}>
            <button type="button" aria-label=${"Wiederherstellen: " + d.titel} onClick=${d.restore} style=${stil(S.buttonReset + 'height:34px;padding:0 14px;border-radius:999px;background:var(--accT);display:flex;align-items:center;gap:6px;font-size:13.5px;font-weight:600;color:var(--acc);cursor:pointer')}>
            ...
            <button type="button" aria-label=${"Endgültig löschen: " + d.titel} onClick=${d.delF} style=${stil(S.buttonReset + 'height:34px;padding:0 14px;border-radius:999px;background:rgba(255,59,48,0.12);display:flex;align-items:center;font-size:13.5px;font-weight:600;color:var(--red);cursor:pointer')}>Endgültig löschen</button>
          </div>
          <div style=${stil(S.trenner)}></div>
        </div>`)}
    </div>
