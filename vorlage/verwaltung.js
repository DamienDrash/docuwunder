...
(function (global) {
  ...
  <div style=${stil('position:absolute;inset:0;overflow-y:auto;padding:112px 0 50px')}>
    ...
    <div style=${stil(S.karte)}>
      ...
      ${v.zweiZeigen ? html`<button type="button" aria-label="Zwei-Faktor-Authentifizierung einrichten oder verwalten" onClick=${v.zweiOeffnen} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px 16px;cursor:pointer;border-top:0.5px solid var(--sep)')}><div style=${stil('flex:1;min-width:0')}><div style=${stil('font-size:15.5px')}>Zwei-Faktor-Authentifizierung</div><div style=${stil('font-size:12.5px;color:var(--lab2);margin-top:2px;line-height:1.45')}>Code aus einer Authenticator-App, zusätzlich zum Passwort</div></div><span style=${stil('font-size:14px;color:var(--acc)')}>Einrichten</span></button>` : null}
