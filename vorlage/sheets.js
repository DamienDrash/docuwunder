...
<div role="dialog" aria-modal="true" aria-label="Dialog" data-sheet-panel tabindex="-1" ref=${v.sheetRef} onKeyDown=${v.sheetKeyDown} style=${stil(`position:absolute;${v.sheetPos}bottom:0;background:var(--card);border-radius:22px 22px 0 0;animation:sheetUp .34s cubic-bezier(.32,.72,.36,1);z-index:61;max-height:78%;display:flex;flex-direction:column;box-shadow:0 -8px 40px rgba(0,0,0,0.18)`)}>
  ...
  ${v.shDatenschutz ? html`<div style=${stil(S.sheetTitel)}>Datenschutz</div>
    ...
    <div style=${stil('padding:14px 16px 0')}>
      <button type="button" aria-label="Quelltext auf GitHub ansehen" onClick=${v.dsOeffnen} style=${stil(S.buttonReset + 'width:100%;' + S.knopf)}>Quelltext ansehen</button>
    </div>` : null}
  ...
  ${v.shAusloeser ? html`<div style=${stil(S.sheetTitel)}>Auslöser</div>
      ...
      ${(v.ausloeserRows || []).map((r, rIdx) => html`<button key=${rIdx} type="button" aria-label=${r.label} aria-pressed=${!!r.on} onClick=${r.pick} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center;padding:13px 20px;cursor:pointer;position:relative')}>
  ...
  ${v.shAdd ? html`<div style=${stil('font-size:18px;font-weight:700;padding:10px 20px 6px')}>Hinzufügen</div>
    <div style=${stil('padding:4px 16px 0;display:flex;flex-direction:column;gap:8px')}>
      <button type="button" aria-label="Dokument scannen" onClick=${v.startScan} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:13px;background:var(--fill);border-radius:14px;padding:13px 14px;cursor:pointer')}>
      ...
      <button type="button" aria-label="Datei hochladen" onClick=${v.pickFile} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:13px;background:var(--fill);border-radius:14px;padding:13px 14px;cursor:pointer')}>
  ...
  ${v.shSort ? html`<div style=${stil('font-size:18px;font-weight:700;padding:10px 20px 6px')}>Sortieren</div>
    ${(v.sortRows || []).map((r, rIdx) => html`<button key=${r && r.id != null ? r.id : rIdx} type="button" aria-label=${r.label} aria-pressed=${!!r.on} onClick=${r.pick} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center;padding:13px 20px;cursor:pointer;position:relative')}>
        <span style=${stil('font-size:16px')}>${r.label}</span>
  ...
  ${v.shFilter ? html`<div style=${stil('display:flex;justify-content:space-between;align-items:center;padding:10px 20px 6px')}>
      ...
      <button type="button" aria-label="Filter zurücksetzen" onClick=${v.resetFilter} style=${stil(S.buttonReset + 'font-size:14.5px;font-weight:600;color:var(--acc);cursor:pointer')}>Zurücksetzen</button>
    </div>
    ...
    <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;padding:0 20px')}>${(v.fArtRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.tap} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>
    ...
    <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;padding:0 20px')}>${(v.fTagRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.tap} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>
    ...
    <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;padding:0 20px')}>${(v.fAbsRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.tap} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>
    ...
    <div style=${stil('display:flex;background:var(--fill);border-radius:10px;margin:0 20px;padding:2px')}>
      ${(v.fZeitRows || []).map((z, zIdx) => html`<button key=${z && z.id != null ? z.id : zIdx} type="button" aria-label=${z.label} aria-pressed=${!!z.on} onClick=${z.tap} style=${stil(S.buttonReset + z.style)}>${z.label}</button>`)}
    </div>
    <button type="button" aria-label=${v.filterApplyLabel} onClick=${v.closeSheet} style=${stil(S.buttonReset + 'width:calc(100% - 40px);margin:20px 20px 0;height:50px;border-radius:14px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer')}>${v.filterApplyLabel}</button>` : null}
  ...
  ${v.shBulkTag ? html`<div style=${stil(S.sheetTitel)}>Schlagwort hinzufügen</div>
    ...
    <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;padding:4px 20px 0')}>${(v.bulkTagRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.tap} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>` : null}
  ...
  ${v.shDocMenu ? html`<div style=${stil('font-size:16px;font-weight:600;padding:10px 20px 6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--lab2)')}>${v.dTitel}</div>
    <button type="button" aria-label=${v.dmFavLabel} onClick=${v.dmFav} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer;position:relative')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--lab)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3.6l2.5 5.2 5.7.7-4.2 3.9 1.1 5.6-5.1-2.8-5.1 2.8 1.1-5.6-4.2-3.9 5.7-.7z"></path></svg><span style=${stil('font-size:16px')}>${v.dmFavLabel}</span><div style=${stil('position:absolute;left:52px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
    <button type="button" aria-label="Erkannten Text anzeigen" onClick=${v.dmText} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer;position:relative')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--lab)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h16M4 12h16M4 17h10"></path></svg><span style=${stil('font-size:16px')}>Erkannten Text anzeigen</span><div style=${stil('position:absolute;left:52px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
    <button type="button" aria-label="Original herunterladen" onClick=${v.dmDownload} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer;position:relative')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--lab)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3.5v11"></path><path d="M8.2 10.7l3.8 3.8 3.8-3.8"></path><path d="M5 20.5h14"></path></svg><span style=${stil('font-size:16px')}>Original herunterladen</span><div style=${stil('position:absolute;left:52px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
    <button type="button" aria-label="Drucken" onClick=${v.dmPrint} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer;position:relative')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--lab)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M7 8V3.5h10V8"></path><rect x="4" y="8" width="16" height="8" rx="1.5"></rect><path d="M7 13.5h10v7H7z"></path></svg><span style=${stil('font-size:16px')}>Drucken</span><div style=${stil('position:absolute;left:52px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
    <button type="button" aria-label="Zuweisen" onClick=${v.dmZuweisen} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer;position:relative')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8.5" r="3.2"></circle><path d="M3.5 19.5c.8-3.5 3-4.9 5.5-4.9s4.7 1.4 5.5 4.9"></path><path d="M17.5 8.5h5M20 6v5"></path></svg><span style=${stil('font-size:16px')}>Zuweisen …</span><div style=${stil('position:absolute;left:20px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
    <button type="button" aria-label="In den Papierkorb" onClick=${v.dmTrash} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 6.5h15"></path><path d="M8.5 6.2V4.5h7v1.7"></path><path d="M6.5 6.5l1 14h9l1-14"></path></svg><span style=${stil('font-size:16px;color:var(--red)')}>In den Papierkorb</span></button>` : null}
  ...
  ${v.shZuweisen ? html`<div style=${stil(S.sheetTitel)}>Zuweisen</div>
    ...
    ${(v.zuweisenZiele || []).map((z, zIdx) => html`<button key=${z && z.id != null ? z.id : zIdx} type="button" aria-label=${`An ${z.name} zuweisen`} onClick=${z.tap} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;align-items:center;gap:12px;padding:13px 20px;cursor:pointer;position:relative')}>
  ...
  ${v.shEdit ? html`<div style=${stil('display:flex;justify-content:space-between;align-items:center;padding:10px 20px 6px')}>
      <button type="button" aria-label="Bearbeiten abbrechen" onClick=${v.eCancel} style=${stil(S.buttonReset + 'font-size:15.5px;color:var(--acc);cursor:pointer')}>Abbrechen</button>
      <span style=${stil('font-size:16.5px;font-weight:700')}>Bearbeiten</span>
      <button type="button" aria-label="Änderungen sichern" onClick=${v.eSave} style=${stil(S.buttonReset + 'font-size:15.5px;font-weight:700;color:var(--acc);cursor:pointer')}>Sichern</button>
    </div>
    <div style=${stil('padding:10px 20px 0')}>
      ...
      <input value=${v.eTitel} onInput=${v.setETitel} aria-label="Titel" style=${stil('width:100%;height:46px;border-radius:12px;border:1px solid var(--sep);background:var(--bg);padding:0 13px;font-size:16px;color:var(--lab);outline:none;margin-top:7px')} style-focus="border-color:var(--acc)" />
      ...
      <input value=${v.eDatum} onInput=${v.setEDatum} aria-label="Ausgestellt am" style=${stil('width:100%;height:46px;border-radius:12px;border:1px solid var(--sep);background:var(--bg);padding:0 13px;font-size:16px;color:var(--lab);outline:none;margin-top:7px')} style-focus="border-color:var(--acc)" />
      <div style=${stil('background:var(--bg);border-radius:12px;margin-top:16px;overflow:hidden')}>
        <button type="button" aria-label=${`Absender ändern, aktuell ${v.eAbs}`} onClick=${v.ePickAbs} style=${stil(S.buttonReset + 'width:100%;display:flex;justify-content:space-between;align-items:center;padding:12px 13px;cursor:pointer;position:relative')}><span style=${stil('font-size:15.5px;color:var(--lab2)')}>Absender</span><span style=${stil('display:flex;align-items:center;gap:6px;font-size:15.5px;font-weight:500')}>${v.eAbs}<svg width="7" height="12" viewBox="0 0 8 14" fill="none" stroke="var(--lab3)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l6 6-6 6"></path></svg></span><div style=${stil('position:absolute;left:13px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
        <button type="button" aria-label=${`Dokumentart ändern, aktuell ${v.eArt}`} onClick=${v.ePickArt} style=${stil(S.buttonReset + 'width:100%;display:flex;justify-content:space-between;align-items:center;padding:12px 13px;cursor:pointer;position:relative')}><span style=${stil('font-size:15.5px;color:var(--lab2)')}>Dokumentart</span><span style=${stil('display:flex;align-items:center;gap:6px;font-size:15.5px;font-weight:500')}>${v.eArt}<svg width="7" height="12" viewBox="0 0 8 14" fill="none" stroke="var(--lab3)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l6 6-6 6"></path></svg></span><div style=${stil('position:absolute;left:13px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
        <button type="button" aria-label=${`Ablageort ändern, aktuell ${v.eOrt}`} onClick=${v.ePickOrt} style=${stil(S.buttonReset + 'width:100%;display:flex;justify-content:space-between;align-items:center;padding:12px 13px;cursor:pointer')}><span style=${stil('font-size:15.5px;color:var(--lab2)')}>Ablageort</span><span style=${stil('display:flex;align-items:center;gap:6px;font-size:15.5px;font-weight:500')}>${v.eOrt}<svg width="7" height="12" viewBox="0 0 8 14" fill="none" stroke="var(--lab3)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l6 6-6 6"></path></svg></span></button>
      </div>
      ...
      <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;margin-top:8px')}>${(v.eTagRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.tap} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>
      ...
      <textarea value=${v.eNotiz} onInput=${v.setENotiz} placeholder="Optional, z. B. wo das Original liegt" aria-label="Notiz" style=${stil('width:100%;height:64px;border-radius:12px;border:1px solid var(--sep);background:var(--bg);padding:10px 13px;font-size:15px;color:var(--lab);outline:none;margin-top:7px;resize:none')} style-focus="border-color:var(--acc)"></textarea>
    </div>` : null}

  ${v.shPick ? html`<div style=${stil('font-size:18px;font-weight:700;padding:10px 20px 6px')}>${v.pickTitle}</div>
    ${(v.pickRows || []).map((r, rIdx) => html`<button key=${r && r.id != null ? r.id : rIdx} type="button" aria-label=${r.label} aria-pressed=${!!r.on} onClick=${r.pick} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center;padding:12px 20px;cursor:pointer;position:relative')}>
        <span style=${stil('font-size:16px')}>${r.label}</span>
        ${r.on ? html`<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.5l4.5 4.5L19 7.5"></path></svg>` : null}
        <div style=${stil('position:absolute;left:20px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div>
      </button>`)}
    ${v.pickCanCreate ? html`<div style=${stil('display:flex;gap:8px;padding:12px 20px 0')}>
        <input value=${v.pickNew} onInput=${v.setPickNew} placeholder=${v.pickNewPlaceholder} aria-label=${v.pickNewPlaceholder} style=${stil('flex:1;height:44px;border-radius:12px;border:1px solid var(--sep);background:var(--bg);padding:0 13px;font-size:15.5px;color:var(--lab);outline:none')} style-focus="border-color:var(--acc)" />
        <button type="button" aria-label="Neuen Eintrag erstellen" onClick=${v.pickCreate} style=${stil(S.buttonReset + 'height:44px;padding:0 16px;border-radius:12px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;font-size:15px;font-weight:600;cursor:pointer')}>Erstellen</button>
      </div>` : null}` : null}
  ...
  ${v.shShare ? html`<div style=${stil(S.sheetTitel)}>Teilen</div>
    ...
    ${v.shNoLink ? html`<div style=${stil('padding:4px 20px 0')}>
        ...
        <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;margin-top:8px')}>${(v.shAblaufRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.pick} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>
        ...
        <div style=${stil('display:flex;gap:7px;flex-wrap:wrap;margin-top:8px')}>${(v.shRechtRows || []).map((c, cIdx) => html`<button key=${c && c.id != null ? c.id : cIdx} type="button" aria-label=${c.label} aria-pressed=${!!c.on} onClick=${c.pick} style=${stil(S.buttonReset + c.style)}>${c.label}</button>`)}</div>
        <button type="button" aria-label="Freigabelink erstellen" onClick=${v.shMake} style=${stil(S.buttonReset + 'width:100%;margin-top:22px;height:50px;border-radius:14px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer')}>Freigabelink erstellen</button>
      </div>` : null}
    ${v.shHasLink ? html`<div style=${stil('padding:4px 20px 0')}>
        <div style=${stil('display:flex;align-items:center;gap:10px;background:var(--bg);border-radius:12px;padding:12px 13px')}>
          ...
          <button type="button" aria-label="Link kopieren" onClick=${v.shCopy} style=${stil(S.buttonReset + 'height:32px;padding:0 13px;border-radius:999px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;font-size:13.5px;font-weight:600;cursor:pointer;flex-shrink:0')}>Kopieren</button>
        </div>
        <div style=${stil('font-size:13px;color:var(--lab2);margin-top:10px')}>${v.shMeta}</div>
        <div style=${stil('background:var(--bg);border-radius:12px;margin-top:16px;overflow:hidden')}>
          <button type="button" aria-label="Als PDF exportieren" onClick=${v.dmDownload} style=${stil(S.buttonReset + 'width:100%;text-align:left;padding:12px 13px;cursor:pointer;position:relative')}><span style=${stil('font-size:15.5px;color:var(--acc)')}>Als PDF exportieren</span><div style=${stil('position:absolute;left:13px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div></button>
          <button type="button" aria-label="Freigabe widerrufen" onClick=${v.shRevoke} style=${stil(S.buttonReset + 'width:100%;text-align:left;padding:12px 13px;cursor:pointer')}><span style=${stil('font-size:15.5px;color:var(--red)')}>Freigabe widerrufen</span></button>
        </div>
      </div>` : null}` : null}
  ...
  ${v.shDelFinal ? html`<div style=${stil('padding:16px 24px 0;text-align:center')}>
      ...
      <button type="button" aria-label="Endgültig löschen" onClick=${v.confirmDelFinal} style=${stil(S.buttonReset + 'width:100%;margin-top:20px;height:50px;border-radius:14px;background:var(--red);color:#fff;display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer')}>Endgültig löschen</button>
      <button type="button" aria-label="Löschen abbrechen" onClick=${v.closeSheet} style=${stil(S.buttonReset + 'width:100%;margin-top:8px;height:46px;border-radius:14px;background:var(--fill);display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:600;color:var(--acc);cursor:pointer')}>Abbrechen</button>
    </div>` : null}

  ${v.shOrgEdit ? html`<div style=${stil('display:flex;justify-content:space-between;align-items:center;padding:10px 20px 6px')}>
      <button type="button" aria-label="Bearbeiten abbrechen" onClick=${v.closeSheet} style=${stil(S.buttonReset + 'font-size:15.5px;color:var(--acc);cursor:pointer')}>Abbrechen</button>
      <span style=${stil('font-size:16.5px;font-weight:700')}>${v.orgEditTitle}</span>
      <button type="button" aria-label="Sichern" onClick=${v.orgSaveTap} style=${stil(S.buttonReset + 'font-size:15.5px;font-weight:700;color:var(--acc);cursor:pointer')}>Sichern</button>
    </div>
    <div style=${stil('padding:10px 20px 0')}>
      <div style=${stil('font-size:12.5px;font-weight:600;color:var(--lab2);text-transform:uppercase;letter-spacing:0.3px')}>Name</div>
      <input value=${v.orgName} onInput=${v.setOrgName} aria-label="Name" style=${stil('width:100%;height:46px;border-radius:12px;border:1px solid var(--sep);background:var(--bg);padding:0 13px;font-size:16px;color:var(--lab);outline:none;margin-top:7px')} style-focus="border-color:var(--acc)" />
      ...
      ${v.orgWarnOn ? html`<div style=${stil('margin-top:14px;background:rgba(255,149,0,0.13);border-radius:14px;padding:12px 14px')}>
          ...
          <button type="button" aria-label="Trotzdem löschen" onClick=${v.orgDelForce} style=${stil(S.buttonReset + 'width:100%;margin-top:10px;height:38px;border-radius:10px;background:var(--red);color:#fff;display:flex;align-items:center;justify-content:center;font-size:14.5px;font-weight:600;cursor:pointer')}>Trotzdem löschen</button>
        </div>` : null}
      ${v.orgDelOn ? html`<button type="button" aria-label="Löschen" onClick=${v.orgDelTap} style=${stil(S.buttonReset + 'width:100%;margin-top:18px;height:46px;border-radius:14px;background:rgba(255,59,48,0.10);display:flex;align-items:center;justify-content:center;font-size:15.5px;font-weight:600;color:var(--red);cursor:pointer')}>Löschen</button>` : null}
    </div>` : null}
  ...
  ${v.shMitgliedNeu ? html`<div style=${stil(S.sheetTitel)}>Mitglied hinzufügen</div>
    ...
    <div style=${stil('padding:0 20px')}>
      ...
      <input value=${v.nmName} onInput=${v.setNmName} placeholder="Anna Beispiel" aria-label="Name" style=${stil(S.feld)} />
      ...
      <input value=${v.nmMail} onInput=${v.setNmMail} placeholder="anna@beispiel.de" aria-label="E-Mail" style=${stil(S.feld)} />
      ...
      ${v.nmGruppenDa ? html`<div style=${stil('font-size:12px;font-weight:600;color:var(--lab2);text-transform:uppercase;letter-spacing:0.3px;padding:14px 0 6px')}>Gruppe</div>
        <div style=${stil('display:flex;gap:8px;flex-wrap:wrap')}>
          ${(v.nmGruppen || []).map((g, gIdx) => html`<button key=${g && g.id != null ? g.id : gIdx} type="button" aria-label=${g.label} aria-pressed=${!!g.on} onClick=${g.pick} style=${stil(S.buttonReset + g.style)}>${g.label}</button>`)}
        </div>` : null}
      ${v.nmErrOn ? html`<div style=${stil('font-size:13px;color:var(--red);margin-top:10px')}>${v.nmErr}</div>` : null}
      <button type="button" aria-label=${v.nmSaveLabel} onClick=${v.nmSave} style=${stil(S.buttonReset + 'width:100%;height:50px;border-radius:12px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;gap:8px;font-size:16.5px;font-weight:600;cursor:pointer;margin:16px 0 30px')}>${v.nmSaveLabel}</button>
    </div>` : null}
  ...
  ${v.shZugang ? html`<div style=${stil(S.sheetTitel)}>Zugang für ${v.zuName}</div>
    ...
    <button type="button" aria-label="Zugangsdaten weitergeben" onClick=${v.zuTeilen} style=${stil(S.buttonReset + 'width:calc(100% - 40px);height:50px;border-radius:12px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;gap:8px;font-size:16.5px;font-weight:600;cursor:pointer;margin:16px 20px 10px')}>
  ...
  ${v.shGruppeNeu ? html`<div style=${stil(S.sheetTitel)}>Gruppe anlegen</div>
    ...
    <div style=${stil('padding:0 20px 30px')}>
      <input value=${v.ngName} onInput=${v.setNgName} placeholder="Buchhaltung" aria-label="Name der Gruppe" style=${stil(S.feld)} />
      <button type="button" aria-label="Gruppe anlegen" onClick=${v.ngSave} style=${stil(S.buttonReset + 'width:100%;height:50px;border-radius:12px;background:var(--acc);color:var(--onAcc);display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer;margin-top:16px')}>Anlegen</button>
    </div>` : null}
  ...
  ${v.shGruppeWeg ? html`<div style=${stil(S.sheetTitel)}>„${v.gwName}“ löschen?</div>
    ...
    <div style=${stil('padding:0 20px 30px')}>
      <button type="button" aria-label="Gruppe löschen" onClick=${v.gwGo} style=${stil(S.buttonReset + 'width:100%;height:50px;border-radius:12px;background:var(--red);color:#fff;display:flex;align-items:center;justify-content:center;font-size:16.5px;font-weight:600;cursor:pointer')}>Gruppe löschen</button>
      <button type="button" aria-label="Löschen abbrechen" onClick=${v.closeSheet} style=${stil(S.buttonReset + 'width:100%;text-align:center;padding:14px;font-size:15.5px;color:var(--lab2);cursor:pointer')}>Abbrechen</button>
    </div>` : null}
  ...
  ${v.shUser ? html`<div style=${stil(S.sheetTitel)}>${v.userName}</div>
    ...
    ${(v.userGruppen || []).map((r, rIdx) => html`<button key=${r && r.id != null ? r.id : rIdx} type="button" aria-label=${r.label} aria-pressed=${!!r.on} onClick=${r.pick} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center;padding:12px 20px;cursor:pointer;position:relative')}>
        <span style=${stil('font-size:16px')}>${r.label}</span>
        ${r.on ? html`<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--acc)" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12.5l4.5 4.5L19 7.5"></path></svg>` : null}
        <div style=${stil('position:absolute;left:20px;right:0;bottom:0;height:0.5px;background:var(--sep)')}></div>
      </button>`)}
    ${v.userAdminZeigen ? html`<button type="button" aria-label="Administrator umschalten" onClick=${v.userAdminToggle} style=${stil(S.buttonReset + 'width:100%;text-align:left;display:flex;justify-content:space-between;align-items:center;gap:12px;padding:14px 20px;cursor:pointer;margin-top:6px;border-top:0.5px solid var(--sep)')}>
    ...
    ${v.userWegZeigen ? html`<button type="button" aria-label="Mitglied entfernen" onClick=${v.userWeg} style=${stil(S.buttonReset + 'width:100%;text-align:left;padding:14px 20px 26px;cursor:pointer;border-top:0.5px solid var(--sep)')}>
