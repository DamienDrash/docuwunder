...
(function (global) {
  ...
  ${v.scanKameraOn ? html`<div data-kamera-vorschau="1" style=${stil('flex:1;position:relative;overflow:hidden;background:#000')}>
      ...
      <div aria-live="polite" data-scan-auto-status="1" style=${stil('position:absolute;left:16px;right:16px;top:18px;text-align:center;font-size:13px;font-weight:600;color:#fff;text-shadow:0 1px 5px rgba(0,0,0,0.8)')}>${v.scanAutoText}</div>
      <button type="button" aria-pressed=${v.scanAutoOn} onClick=${v.scanAutoToggle} style=${stil(S.buttonReset + 'position:absolute;right:16px;top:54px;height:36px;padding:0 13px;border-radius:18px;background:' + (v.scanAutoOn ? 'var(--acc);color:var(--onAcc)' : 'rgba(120,120,128,0.32);color:#fff') + ';font-size:13px;font-weight:700;cursor:pointer')}>Auto</button>
